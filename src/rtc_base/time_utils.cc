...
class TimeHelper final {
 public:
  ...
  static void SyncWithNtp(int64_t ntp_server_time_ms) {
    ...
    int64_t time_zone_bias_ns =
        std::static_cast<int64_t>(time_zone.Bias) * 60 * 1000 * 1000 * 1000;
  ...
  static int64_t TicksNs() {
    ...
    result = std::static_cast<int64_t>(
        (std::static_cast<uint64_t>(qpcnt.QuadPart) * 100000 /
         std::static_cast<uint64_t>(singleton.os_ticks_per_second_)) *
        10000);
 ...
 private:
  ...
  void UpdateReferenceTime() {
    ...
    os_ticks_per_second_ = std::static_cast<int64_t>(qpfreq.QuadPart);

    LARGE_INTEGER qpcnt;
    QueryPerformanceCounter(&qpcnt);
    time_since_os_start_ns_ = std::static_cast<int64_t>(
        (std::static_cast<uint64_t>(qpcnt.QuadPart) * 100000 /
         std::static_cast<uint64_t>(os_ticks_per_second_)) *
        10000);
  }
